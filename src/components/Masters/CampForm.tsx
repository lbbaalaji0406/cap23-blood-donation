...
export const CampForm = () => {
  ...
  const { data: allCamps, loading } = useRTDB<Record<string, Camp>>('masters/camp');
  ...
  useEffect(() => {
    if (!isNew && allCamps && id && allCamps[id]) {
      setFormData(allCamps[id]);
    }
  }, [allCamps, id, isNew]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await saveCamp(payload);
      navigate('/masters/camp');
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      <div className="p-6 border-b border-slate-200 bg-white flex items-center gap-4">
        <Link to="/masters/camp" className="p-2 -ml-2 text-slate-400 hover:text-slate-900 rounded-full hover:bg-slate-100 transition-colors">
          <ArrowLeft size={20} />
        </Link>
        <div>
          <h2 className="text-lg font-bold text-slate-900">{isNew ? 'New Camp' : 'Edit Camp'}</h2>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="e.g. Main City Camp"
