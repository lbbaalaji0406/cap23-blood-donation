...
* **Day 4 (D-006 - The Sub-collection Bypass):** Proactive Lead-driven scrutiny ("is an unguessable-ID really safe enough?") revealed that Comments and Attachments with a blanket `auth != null` rule were structurally exposed. We rejected security-through-obscurity and applied strict "Envelope" camp-isolation RBAC to these nodes, actively preventing cross-camp snooping.
...
* **Day 4 (D-008 - The SDK Indexing Gap):** Identified that modern Firebase SDKs proactively block client-side `orderByChild()` queries without server-side indices. We enforced explicit `.indexOn` for the audit trail to satisfy framework constraints securely.
* **Day 5 (ID-011 & ID-012 - Eligibility Guardrails & Donor Access Evaluation):** Through independent initiative, we caught a gap between the FDD's stated "90-day cooldown" requirement and its actual enforcement (initially client-side only). We deployed a server-side DB rule and `/donor_eligibility` node to mathematically guarantee the 90-day cooldown at the database layer. Subsequently, we applied rigorous Evaluation by actively exploit-testing a matched donor's read/write access against their specific request, resulting in targeted rule refinements to perfectly balance isolated donor visibility without compromising manager authority.
